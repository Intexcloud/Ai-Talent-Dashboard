...
def run_talent_match_query(_conn, bench_ids, weights):
    ...
    Menggunakan _conn agar Streamlit tidak mencoba meng-hash objek koneksi.
    """ 
    if not _conn:
    ...
    try:
        ...
        # Menggunakan _conn di sini
        df_results = pd.read_sql(query, _conn, params=(bench_ids, weights_json))
...
# Memperbaiki pemanggilan get_employee_list agar menggunakan underscore jika conn didefinisikan dengan underscore
df_employees = get_employee_list(_conn=conn) 
...
if run_button:
    if not conn:
        ...
        with st.spinner("Mencari kandidat terbaik..."):
            # KOREKSI INI: Ganti conn menjadi _conn=conn saat memanggil fungsi
            results_df = run_talent_match_query(_conn=conn, bench_ids=selected_talent_ids, weights=weights_config) 
